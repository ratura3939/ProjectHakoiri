...
public:
	...
	const bool FindPlayer(const Vector2F pPos)const;	//視野内にplayerがいるか

	void SetIsUse(const bool flag);	//敵を使用しているかをセット
	const bool IsUse(void);			//敵が使用済みかを確認
	const TYPE GetType(void);			//敵の種類を獲得
	void SetIsMove(bool flag);	//動いているかをセット	
	void SetVisionState(const VISION_STATE state);	//敵の視覚状態をセット
	void SetColObject(const Vector2 obj);	//視界を遮るオブジェクトの位置（配列指数）をセット
	void ResetColObject(void);				//視界を遮るオブジェクトの位置（配列指数）をリセット
	const Vector2 GetColObject(void)const;		//視界を遮るオブジェクトの位置（配列指数）をゲット
