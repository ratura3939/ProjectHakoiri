...
void InputManager::Update(void)
{
	...
	GetMousePoint(&mousePos_.x_, &mousePos_.y_);
...
InputManager::JOYPAD_IN_STATE InputManager::GetJPadInputState(JOYPAD_NO no)
{
	...
	// dxlibのバージョンの違いでパッドのタイプが違うことが発生する
	...
	case InputManager::JOYPAD_TYPE::SWITCH_PRO_CTRL:
	{
		...
		idx = static_cast<int>(JOYPAD_BTN::DOWN);
		ret.ButtonsNew[idx] = d.Buttons[0];// B

		idx = static_cast<int>(JOYPAD_BTN::RIGHT);
		ret.ButtonsNew[idx] = d.Buttons[1];// A
		...
		idx = static_cast<int>(JOYPAD_BTN::ZL_TRIGGER);
		ret.ButtonsNew[idx] = d.Buttons[6];// ZL

		idx = static_cast<int>(JOYPAD_BTN::ZR_TRIGGER);
		ret.ButtonsNew[idx] = d.Buttons[7];// ZR
