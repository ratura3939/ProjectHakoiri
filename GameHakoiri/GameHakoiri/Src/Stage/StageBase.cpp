...
{
	//指定した場所にオブジェクトがあったら
	if (roomMng_[roomKey_]->GetObj(pMapPos) != -1){	
		return true;
	}
	return false;
}
...
{
	//移動先が壁・ゴールだったら処理は市内
	if (IsDontMoveBlock(aft)) { return false; }
...
const bool StageBase::CheckInstanceUp(int y, const int x, RoomBase& r)
{
	...
	if (roomMng_[roomKey_]->GetRoomType() == r.GetRoomType()){return true;}
